...
void error(const char* func, int error_num){
    cerr << func << ": " << strerror(error_num) << endl;
    exit(-1);
}
...
void del(int id){
    ...
    for(int i = 0; i < (int)bp_list.size(); i++){
        if(bp_list[i].id == id){
            ...
            for(int j = i; j < (int)bp_list.size(); j++)
                bp_list[j].id = id + j - i;
...
void get(string reg){
    ...
    if(ptrace(PTRACE_GETREGS, pid, 0, &regs) != 0)    
        error("ptrace(PTRACE_GETREGS)", errno);
    if(reg == "rax")      val = regs.rax;
    else if(reg == "rbx") val = regs.rbx;
    else if(reg == "rcx") val = regs.rcx;
    else if(reg == "rdx") val = regs.rdx;
    else if(reg == "r8")  val = regs.r8;
    else if(reg == "r9")  val = regs.r9;
    else if(reg == "r10") val = regs.r10;
    else if(reg == "r11") val = regs.r11;
    else if(reg == "r12") val = regs.r12;
    else if(reg == "r13") val = regs.r13;
    else if(reg == "r14") val = regs.r14;
    else if(reg == "r15") val = regs.r15;
    else if(reg == "rdi") val = regs.rdi;
    else if(reg == "rsi") val = regs.rsi;
    else if(reg == "rbp") val = regs.rbp;
    else if(reg == "rsp") val = regs.rsp;
    else if(reg == "rip") val = regs.rip;
    else if(reg == "flags") val = regs.eflags;
    else{
...
void vmmap(){
    ...
    size_t len;
    FILE *fp;
    if((fp = fopen(dir.c_str(), "r")) == NULL)  error("fopen", errno);
    while(getline(&line, &len, fp) != EOF){
    //  line sample: 00600000-00601000 rw-p 00000000 08:02 2622929   /home/usr/unix_prog/hw4/sample/hello64
        ...
        if((token = strtok(NULL, " ")) != NULL)     // /home/usr/unix_prog/hw4/sample/hello64    pathname
            cerr << string(token);
    }
}
...
void run(){
    if(state == RUNNING){
        ...
        cout << "** state must be RUNNING or LOADED\n";
    }
}
void start(){
    if(state != LOADED){
        cout << "** state must be LOADED\n";
        return;
    }
    if(pid){
        cerr << "** program " << file_path << " is already running\n";
        return;
    }
    if((pid = fork()) < 0)  
        error("fork", errno);
...
void load(string arg){
    if(state != NONLOAD){
        cout << "** state must be NOT LOADED\n";
        return;
    }
    if(arg.empty()){
        cout << "** no program path is given\n";
        return;
    }
    FILE *fp;
    elf.str_tab = NULL;
    int str_shdr_offset;
    if((fp = fopen(arg.c_str(), "rb")) == NULL)
        error("fopen", errno);
    elf.fp = fp;
    ...
    cout << "** program '" << arg << "' loaded. entry point 0x" << hex << elf.ehdr.e_entry << "\n";
    file_path = arg;
    state = LOADED;
}
...
int main(const int argc, const char *argv[]){
    ...
    for(int i = 1; i < argc; i++){
        if(strcmp(argv[i], "-s") == 0){   //[-s script]
            string script_path = argv[++i];
            f.open("./" + script_path, ifstream::in);
            if (!f){
                cerr << script_path << ": " << strerror(errno) << endl;
                exit(-1);
            }
        }else{                          //[program]
            load(argv[i]);
    ...
    while(1){
        ...
        if(cmd == "break" || cmd == "b"){
            ...
            if(!arg1.size()){
                cout << "** no break-point-id is given\n";
                continue;
            }
            del(atoi(arg1.c_str()));
